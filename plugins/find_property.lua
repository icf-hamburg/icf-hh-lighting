...
local function user_input()
    ...
    if resultTable.success and resultTable.result == 1 then
        return resultTable.selectors['Search for property value ot property name?'],
            resultTable.inputs['Where to search'], resultTable.inputs[' What to search'],
            resultTable.states['Exact match  ']
    end
end
...
local function find_property(display)
    ...
    if what_to_search then
        ...
        Echo('Have searched for the property ' .. value_or_name .. ' ' .. CY .. what_to_search ..
        YE .. ' in ' .. MG .. obj_name)
        ...
        if #properties ~= 0 then
            if Confirm('Store .csv file with search results?') then
                ...
                path = GetPath(Enums.PathType.Library):gsub('/', GetPathSeparator()) ..
                GetPathSeparator() .. 'search_plugin_results'
                CreateDirectoryRecursive(path)
                local filename_time = string.format('%02d-%s-%sT%02d-%02d-%02d', time.day, time.month:sub(1, 3),
                    tostring(time.year):sub(-2, -1), time.hour, time.minute, time.second)
                path = path ..
                GetPathSeparator() ..
                "searched_in_" ..
